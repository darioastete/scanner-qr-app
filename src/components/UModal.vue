  ...
  <transition name="slide-up" appear>
    ...
    >
      ...
      <div
        ...
        :class="{
          dragging: isDragging,
          '!right-1/2 !top-1/2 !h-fit !w-[360px] !-translate-y-1/2 !translate-x-1/2 !transform !rounded-[4px] !px-[24px] !py-[36px]':
            !hasMobileMode,
          'max-w-[90%]': !hasMobileMode,
        }"
        class="absolute bottom-0 z-10 flex w-full flex-col gap-[24px] rounded-t-[20px] border-[1px] border-neutrals-sky-400 bg-white p-[16px] pt-[8px] shadow-popup sm:right-1/2 sm:top-1/2 sm:h-fit sm:w-[360px] sm:-translate-y-1/2 sm:translate-x-1/2 sm:transform sm:rounded-[4px] sm:p-[24px] sm:pt-[36px]"
      >
        <!-- Header -->
        <div
          v-if="hasBackIcon || title !== '' || hasCloseIcon || hasMobileMode"
          class="flex flex-col"
        >
          ...
          <div
            v-if="hasBackIcon || title !== '' || hasCloseIcon"
            class="flex items-center justify-between"
          >
            <div class="h-[24px] w-[24px]">
              <button v-if="hasBackIcon">
                <!-- <img
            ...
            <div>
              <p
                class="text-center text-sm font-semibold text-neutrals-plate-400"
              >
                {{ title }}
              </p>
            </div>
...
let startY = 0
let currentY = 0
let modalTranslateY = ref("") // Inicializa la variable reactiva para la transformación
